      ...
      <div class="booking-grid">
          ...
          <div class="days">
            <!-- <button
              ...
              :class="{ 
                ...
                available: day.date && !day.isPast && day.hasSlots
              }"
        ...
        <div class="booking-form">
          ...
          <div v-else class="form-content">
            ...
            <div class="time-slots" v-if="availableSlots.length">
              <h3>Horários</h3>
...
const calendarDays = computed(() => {
  ...
  days.forEach(day => {
    ...
    const hasSlots = store.value.available_dates?.includes(dateStr) || false
    cells.push({
      key: day.toISOString(),
      num: day.getDate(),
      date: day,
...
const fetchStore = async () => {
  loading.value = true
  try {
    const res = await axios.get(`${API_URL}/user/store/detail/${storeId}`)
...
const fetchAvailableSlots = async () => {
  ...
  try {
    ...
    const res = await axios.get(`${API_URL}/store/available-times/${storeId}/${dateStr}`)
...
const selectDate = (date: Date | null) => {
  ...
  console.log('s: ', date, store.value.available_dates)
  if (!date || isPast(date) || !store.value.available_dates?.includes(format(date, 'yyyy-MM-dd'))) return
...
@media (max-width:968px){
  ...
  .calendar-section,.booking-form{border-radius:24px}
}
